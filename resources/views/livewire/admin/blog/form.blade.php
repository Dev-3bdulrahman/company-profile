        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {{ $post_id ? __('Edit Post') : __('Add Post') }}
                </h2>
                <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('Write and publish your blog post') }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <select wire:model="status"
                class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none font-semibold">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                <div class="flex border-b border-gray-100 dark:border-gray-800">
                    <button wire:click="$set('active_lang', 'ar')"
                        onclick="editorSwitchLang('blog-editor', 'ar')"
                    ...
                    <button wire:click="$set('active_lang', 'en')"
                        onclick="editorSwitchLang('blog-editor', 'en')"
                ...
                <div class="p-6 space-y-4">
                    {{-- Title --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('Title') }}</label>
                        @if($active_lang === 'ar')
                        <input wire:model="title.ar" type="text" dir="rtl" placeholder="{{ __('Post title in Arabic') }}"
                        ...
                        @else
                        <input wire:model="title.en" type="text" dir="ltr" placeholder="{{ __('Post title in English') }}"
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-lg font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all">
                    ...
                    {{-- Excerpt --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('Excerpt') }}</label>
                        @if($active_lang === 'ar')
                        <textarea wire:model="excerpt.ar" rows="2" dir="rtl" placeholder="{{ __('Short summary...') }}"
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all resize-none"></textarea>
                        @else
                        <textarea wire:model="excerpt.en" rows="2" dir="ltr" placeholder="{{ __('Short summary...') }}"
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all resize-none"></textarea>
                        @endif
                    </div>
                    ...
                    {{-- Rich Text Editor --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('Content') }}</label>
                        ...
                        <div class="tiptap-editor-wrap border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                            ...
                            <div class="flex flex-wrap gap-1 p-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <button type="button"
                                    onmousedown="event.preventDefault(); editorCmd('blog-editor', '{{ $action }}')"
                                ...
                                <button type="button"
                                    onmousedown="event.preventDefault(); document.getElementById('blog-img-input').click()"
                                    class="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 transition-colors"
                                    title="{{ __('Insert Image') }}">
                                    <i data-lucide="image" class="w-4 h-4"></i>
                                </button>
                                <input type="file" id="blog-img-input" class="hidden" accept="image/*"
                                    onchange="editorUploadImg('blog-editor', this.files[0]); this.value=''">
                            </div>

                            {{-- Editor Container --}}
                            <div wire:ignore id="blog-editor" class="min-h-[300px] bg-white dark:bg-gray-900"></div>
                        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    <input type="file" wire:model="featured_image" id="blog-cover" accept="image/*" class="hidden">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                <div class="p-5 border-b border-gray-50 dark:border-gray-800 flex items-center gap-3">
                    ...
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm">{{ __('Post Settings') }}</h3>
                </div>
                <div class="p-5 space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 tracking-wider mb-1.5">{{ __('slug') }}</label>
                        <input wire:model="slug" type="text" dir="ltr" placeholder="{{ __('post-url-slug') }}"
                            class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Category') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Publish Date') }}</label>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                <div class="p-5 border-b border-gray-50 dark:border-gray-800 flex items-center gap-3">
                    ...
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm">{{ __('Tags') }}</h3>
                </div>
                <div class="p-5 flex flex-wrap gap-2">
                    @foreach($tags as $tag)
                    <label class="inline-flex items-center gap-1.5 cursor-pointer px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 hover:border-blue-400 transition-colors {{ in_array($tag->id, $selected_tags) ? 'bg-blue-50 border-blue-400 dark:bg-blue-900/20' : '' }}">
                        <input type="checkbox" wire:model="selected_tags" value="{{ $tag->id }}" class="hidden">
                        <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ is_array($tag->name) ? ($tag->name[app()->getLocale()] ?? reset($tag->name)) : $tag->name }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
